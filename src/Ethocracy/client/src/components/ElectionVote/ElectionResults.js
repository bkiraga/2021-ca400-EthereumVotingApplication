...
import { maskBallot, unmaskBallot } from "../../election_tools/Encrypt";
...
class ElectionResults extends Component {
    ...
    async handleUnlockResults() {
      ...
      const testCiphertext = maskBallot("teststring", electionKey);
      try {
        if (unmaskBallot(testCiphertext, this.state.resultKey) === "teststring") {
    ...
    async unmaskElectionBallots() {
      ...
      for (let i = 0; i < this.state.ballotCount; i++){
        let unmaskedBallot = unmaskBallot(this.state.ballots[i], resultKey);
    ...
    async handleElectionResults() {
      ...
      if (this.state.electionType == "FPP") {
        ...
        this.setState(() => {
          ...
          console.log(unformatedSTVBallots);
